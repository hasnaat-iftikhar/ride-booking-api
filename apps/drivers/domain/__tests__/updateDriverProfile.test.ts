...
import { riderDataAccess } from '../../../riders/data-access/riderDataAccess'; // Though not directly used by updateDriverProfile
...
describe('DriverService - updateDriverProfile', () => {
  ...
  describe('updateDriverProfile', () => {
    const driverId = 'driver-update-test-id';
    const updatePayload: Partial<DriverType> = {
    ...
    const mockUpdatedDriverFromDb: DriverType = {
      ...
      password: 'hashedOrOriginalPassword',
      status: 'online',
      created_at: new Date(Date.now() - 86400000),
      updated_at: new Date(),
    };

    it('should update driver profile and return updated data without password', async () => {
      mockedDriverDataAccess.updateDriver.mockResolvedValue(mockUpdatedDriverFromDb);
      const result = await driverService.updateDriverProfile(driverId, updatePayload);
      const { password, ...expectedDriver } = mockUpdatedDriverFromDb;
      expect(result).toEqual(expectedDriver);
      expect(result).not.toHaveProperty('password');
      expect(mockedDriverDataAccess.updateDriver).toHaveBeenCalledWith(driverId, updatePayload);
      expect(mockedThrowError).not.toHaveBeenCalled();
    });

    it('should call throwError with NOT_FOUND if driver to update is not found', async () => {
      mockedDriverDataAccess.updateDriver.mockResolvedValue(null);
      await expect(driverService.updateDriverProfile(driverId, updatePayload))
